...
If $Q_s(h)\propto P(h)e^{-\lambda_t\hat{\mathcal{L}}(h,S_s)}$, we get through a similar analysis and using Hoeffding's inequality:
...
We can completely remove covariance terms, leading to improved bounds:
...
We can provide oracle bounds on the forgetting of $\hat{Q}^{\lambda_T}_{1:T}$:
...
we can remove that term and apply Jensen's inequality, giving us
...
This implies that we want to pick hypotheses that are at least orthogonal w.\!r.\!t.\! future tasks in order avoid forgetting. The same condition also applies for generalization, thus suggesting that we want to pick updates that are at least orthogonal to the previous losses. Methods such as OGD \citep{farajtabar2020orthogonal} try to apply a similar notion to gradient descent in order for updates to be orthogonal to previous tasks. This bound suggests that this orthogonality not required if tasks are positively correlated.
...
$$\forall i,j\in [1,T-1]: \mathrm{cov}_{P}(e^{-\lambda_T\hat{\mathcal{L}}(h,S_T)}, e^{-\lambda_j\hat{\mathcal{L}}(h,S_j)})\ll 0, \mathrm{cov}_{P}(e^{-\lambda_i\hat{\mathcal{L}}(h,S_i)}, e^{-\lambda_j\hat{\mathcal{L}}(h,S_j)})\gg 0$$
...
From these two equations and our correlation condition, we can clearly see that the Gibbs learning process has a trade-off: if $\hat{\mathcal{L}}(P,S_T)$ is low, $\sum_{i=1}^{T-1}\hat{\mathcal{L}}(P,S_i)$ is high and vice-versa.
