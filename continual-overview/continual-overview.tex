...
While this equation is not easy to interpret, we note that we can re-write the second term as $\mathbb{E}_{h\sim Q'_{s:t}}\left [\mathcal{L}(h,\mathcal{D}_s) \right ]$, where $$Q'_{s:t}(h)=\frac{Q_s(h)e^{\lambda(\mathcal{L}(h,\mathcal{D}_s)-\mathcal{L}(h,\mathcal{D}_t))}}{\mathbb{E}_{h\sim Q_s}\left [e^{\lambda(\mathcal{L}(h,\mathcal{D}_s)-\mathcal{L}(h,\mathcal{D}_t))}\right ]}$$.
If, for example, $Q_s(h)\propto P(h)e^{-\lambda\mathcal{L}(h,\mathcal{D}_s)}$, we have $Q'_{s:t}(h)\propto  P(h)e^{-\lambda\mathcal{L}(h,\mathcal{D}_t)}$, giving us a bound on backward transfer in terms of a learner that did not use any data from the source task.
...
\begin{enumerate}
	\item The first multiplicative term connects the exponential moments of the discrepancies. This term is lower bounded by $1$, and is upper bounded (using Bergstrom's inequality) by $$\max\left \{{\mathbb{E}_{Q_s}\left [e^{\lambda(\mathcal{L}(h,\mathcal{D}_s)-\mathcal{L}(h,\mathcal{D}_t))}\right ]},\quad e^{\lambda(\mathcal{L}(Q_s,\mathcal{D}_t)-\mathcal{L}(Q_s,\mathcal{D}_s))}\right \}$$ 
	where the first term applies if $\mathbb{E}_{Q_s}\left [e^{2\lambda(\mathcal{L}(h,\mathcal{D}_s)-\mathcal{L}(h,\mathcal{D}_t))}\right ]\geq 1$, and the second term if it is in $(0,1)$. In both cases, we have a domain discrepancy term that refers to the averaged loss discrepancy under $Q_s$. Since we can expect $Q_s$ to give high probability to hypotheses with low loss on $\mathcal{D}_s$, we are likely to be in the second setting and have this term be dominated by the forward transfer term in $e^{\lambda\mathcal{L}(Q_s,\mathcal{D}_t)}$.
...
We can also use the Bhatia-Davis inequality to replace the second and third terms of \eqref{eq:oracle-final} with $\sqrt{K\mathcal{L}(Q_s,\mathcal{D}_s)}$ if we prefer to avoid variance terms.
