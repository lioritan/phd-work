...
The PAC-Bayes framework, as formulated by \citet{Mcallester}, takes as input the training data $S$ as well as an inductive bias in the form of a prior distribution $P$ over $\mathcal{H}$. These are then used to construct a posterior distribution $Q$ over $\mathcal{H}$, and $h\sim Q$ is then sampled.
...
$$\mathcal{L}(Q, \mathcal{D})\triangleq \Expect{h\sim Q}{\Expect{z\sim \mathcal{D}}{\ell(h,z)}}$$
$$\hat{\mathcal{L}}(Q, S)\triangleq \Expect{h\sim Q}{\frac{1}{m}\sum_{i=1}^{m} \ell(h,z_i)}$$.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.4\textwidth]{setup_ml.PNG}
...
\begin{theorem} (Meta-learning bound \citep{Amit2018}) \label{thm:meta-pb}
	...
	&+\sqrt{\frac{\texttt{KL}(\mathcal{Q}||\mathcal{P})+\log\frac{2N}{\delta}}{2(N-1)}} \\
	&+\frac{1}{N}\sum_{i=1}^{N}\sqrt{\frac{\texttt{KL}(\mathcal{Q}||\mathcal{P})+\Expect{P\sim \mathcal{Q}}{\texttt{KL}(Q_i||P)}+\log\frac{2Nm}{\delta}}{2(m-1)}}
	\end{split}
	\end{align}
	
\end{theorem}

Where $Q_i\triangleq Q(P,S_i)$ and $\texttt{KL}$ is the $D_{KL}$ function. This bound on the transfer error contains two complexity terms: an environment-level complexity term that decreases as $N\rightarrow \infty$, and a second task-level term that decreases as $m\rightarrow \infty$. A potentially tighter and simpler version of this bound was recently proposed by \cite{Rothfuss2020}.
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=0.75\textwidth]{data_dependant_adaptation.PNG}
...
\begin{align*}
\begin{split}
&\min_{\mathcal{Q}} \left\{ \hat{\mathcal{L}}(\mathcal{Q}, S_T) + \frac{1}{\lambda}D_{KL}(\mathcal{Q}||\mathcal{Q}_{1:N}) \right\} = \frac{\mathcal{Q}_{1:N}e^{-\lambda\hat{L}(Q,S_T)}}{\Expect{P\sim \mathcal{Q}_{1:N}}{e^{-\lambda\hat{L}(Q,S_T)}}}\\
&\triangleq \mathcal{Q}^{\lambda}_{1:N,T}
\end{split}
\end{align*}
...
\begin{defn}
	We define the adapted meta-loss (AML) 
	\begin{align*} 
	\begin{split}
	...
	\end{split}
	\end{align*} as the fully adapted loss, meaning $S_T$ is used to adapt both the hyper-prior ($\mathcal{Q}_{1:N}$) and the sampled prior. $\hat{\mathcal{L}}_{\mathrm{aml}}$ is a function of both $\gamma$ and $\beta$. 
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.45\textwidth]{toy_example_train.JPG}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.49\textwidth]{toy_example_erm.JPG}
...
%\begin{split}
%&\hat{\mathcal{Q}},\hat{Q}=\\
%&\arg\min_{\mathcal{Q},Q}\left [\hat{\mathcal{L}}(\mathcal{Q}, S_T) + \frac{1}{\lambda}\Expect{P\sim \mathcal{Q}}{D_{KL}(Q||P)}+\frac{1}{\lambda}D_{KL}(\mathcal{Q}||\mathcal{P})\right ]
%\end{split}
%\end{align*}
...
%\begin{split}
%&\mathcal{L}(\hat{\mathcal{Q}},\mathcal{D}_T)\leq \hat{l}_{\mathrm{train}} + \sqrt{\frac{\mathrm{KL}(\mathcal{Q}_{1:N}||\mathcal{P})+\log2N/\delta}{2(N-1)}}\\
%&+O\left (\sqrt{\frac{\log{1/\delta_T}}{m}}\right ) \\
%&+\frac{1}{N}\sum_{i=1}^{N}\sqrt{\frac{\mathrm{KL}(\mathcal{Q}_{1:N}||\mathcal{P})+\Expect{P\sim \mathcal{Q}_{1:N}}{\mathrm{KL}(Q_i||P)}+\log{2Nm/\delta}}{2(m-1)}}
%\end{split}
%\end{align}
%
%
...
Algorithm \ref{alg1} describes the meta-adaption process with a changing hyper-prior, as it is the most complex of these settings, and the other options compare the hyper-posterior to a predefined (data-free) given hyper-prior.
			...
			\While {Not converged} \Comment Or limit number of epochs
				\For {each meta-batch from $1$ to $k$} 
					...
					\For {each Monte-Carlo estimation} 
						...
						\State Calculate $\hat{L}(h,S_{\mathrm{eval}})$ and add to losses
					\EndFor 
...
We perform our tests on tasks with 100 shuffled pixels, as well as tasks with permuted labels instead of pixels.
...
\begin{table*}[t]	
	...
	\begin{tabular}{llll}
		...
		Adaptive, constant hyper-prior & 20   & $0.896\pm 0.012 $ & $0.754\pm 0.093 $ \\
		...
		\bottomrule
	\end{tabular}
	\caption{Test accuracies for the meta-MNIST dataset with 100 shuffled pixels or permuted labels. All accuracies are averaged over 10 seeds and reported with standard error.}
	\label{table:gamma}
\end{table*}

As we can see from Figure \ref{fig:results-pixels}, for the shuffled-pixels task, meta-adaptation with respect to a constant hyper-prior $N(0,\sigma I_d)$ yields a hyper-posterior that is both better in terms of overall test accuracy, as well as the number of gradient steps required to achieve said accuracy. % TODO-reasons
Additionally, for the shuffled-pixels task with a varying hyper-prior that is initialized as the meta-training hyper-prior, we can see similar levels of improvement, but only for larger sample sizes for the adaptation set $S_T$. The large improvement in accuracy between 5 and 40 meta-adaptation steps suggests that given a larger number of meta-adaptation steps, an even better hyper-prior can be found.
