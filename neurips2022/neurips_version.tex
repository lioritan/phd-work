...
This moment-generating function intuitively quantifies the concentration of the function $f$ in the stochastic kernel $P\in{\cal K}({\cal Z}^m,{\cal H})$ \RM{$P\in{\cal K}({\cal Z}^m,{\cal H})$}, and will be low if $f$ is well-concentrated.
The term $\log\xi(P_S, \mathcal{D}, f)$ is known as the \emph{log-moment}. One particularly appealing application of this general bound is for learners that conform to certain algorithmic stability properties that cause the log-moment term to be bounded, and therefore yield a useful upper bound on the expected error. As an example for this notion, \citet{Rivasplata2020} show that for any prior satisfying the differential privacy property ($DP(\epsilon)$), the log-moment can be upper bounded by the log-moment of a data-free prior plus a term that depends on the privacy parameter $\epsilon$. \RM{Give ref and examples} \LF{Gave one example, can also give \citet{Farid2021} (PAC-BUS) as an example, but it's a training set bound so it's hard to compare directly.}
...
It is important to note that the log-moment term $\log\tilde{\xi}(\lambda,\mathcal{Q}_{1:N},\mathcal{D}_T)$ is a key term in both cases. This term will be low if the empirical losses are well concentrated around the expected loss for our data-dependent prior. One possible method of achieving this is by choosing a base learner $Q$ with algorithmic stability properties.
...
We consider the test performance of this hyper-posterior on a new test task $\mathcal{D}_T$, which in this case is out-of-distribution. Figure \ref{fig:ex-erm} shows the result of minimizing the empirical loss on a sampled test set $S_5$ (i.e. running ERM starting from a prior sampled from the hyper-prior $P\sim \mathcal{Q}_{1:4}$). We see that for this simple setting, the empirical loss for the posterior is low, as there is sufficient data to learn the new task.
...
$$\min_{Q}\hat{\mathcal{L}}(Q, S_T) + \frac{1}{\lambda}KL(Q||P)+\frac{1}{\lambda}KL(\mathcal{Q}||\mathcal{Q}_{1:4})$$
where unlike before, the task prior $P$ is sampled from $\mathcal{Q}$. Figure \ref{fig:ex-aml} shows the adaptation process as well as the final result. We see that this objective yields a hyper-posterior $\mathcal{Q}$ with a closer mean to the test task, and we see that the variance of the prior is high along the axis connecting the hyper-prior and the test posterior. 
...
\LF{Possible critique: test task is OOD so training bound assumptions are slightly violated. This still works if we have 3 training tasks and the fourth one completes the corners}

Since the empirical loss of the final hypothesis is similar regardless of hyper-prior, we can easily compare the bounds. We will mark this empirical loss $\hat{l}$. For the training-based bound we have (with probability at least $1-\delta$ over the choice of $S_1,...,S_4$ and probability at least $1-\delta_T$ over the choice of $S_T$):

\begin{align*}
\begin{split}
	\mathcal{L}(\mathcal{Q}_{1:4},\mathcal{D}_T)\leq \hat{l} &+ \sqrt{\frac{D_{KL}(\mathcal{Q}_{1:4}||\mathcal{P})+\log\frac{8}{\delta}}{6}}\\
	&+\frac{1}{4}\sum_{i=1}^{4}\sqrt{\frac{ D_{KL}(\mathcal{Q}_{1:4}||\mathcal{P})
			+\Expect{P\sim\mathcal{Q}}{D_{KL}(Q(P,S_i)||P)}+\log\frac{8000}{\delta}}{2000}}\\
		&+\sqrt{\frac{\log\frac{1}{\delta_T}}{2000}}
\end{split}
\end{align*}


We note that the KL-divergences for each task are identical (and empirically, are less than 1). This gives us a bound of form:

$$	\mathcal{L}(\mathcal{Q}_{1:4},\mathcal{D}_T)\leq \hat{l} + \sqrt{\frac{D_{KL}(\mathcal{Q}_{1:4}||\mathcal{P})+3+\log\frac{1}{\delta}}{6}}
+\sqrt{\frac{ D_{KL}(\mathcal{Q}_{1:4}||\mathcal{P})
		+1+13+\log\frac{1}{\delta}}{2000}}
+\sqrt{\frac{\log\frac{1}{\delta_T}}{2000}}$$

We can clearly see that the dominant term is $\hat{l} + \sqrt{\frac{D_{KL}(\mathcal{Q}_{1:4}||\mathcal{P})+3+\log\frac{1}{\delta}}{6}}$.  

A quick glance at the meta-adaptation bound shows us that we have for $\lambda=\sqrt{m}$ (with probability at least $1-\delta_T$ over the choice of $S_T$):

\begin{align*}
\begin{split}
\mathcal{L}(\mathcal{Q},\mathcal{D}_T)\leq \hat{l} &+\frac{1}{\sqrt{1000}}D_{KL}(\mathcal{Q}||\mathcal{Q}_{1:4})+\frac{1}{\sqrt{1000}} \log\frac{\tilde{\xi}(\sqrt{1000},\mathcal{Q}_{1:4},\mathcal{D}_T)}{\delta_T}
\end{split}
\end{align*}

The dominant term for this case is $\hat{l} + +\frac{1}{\sqrt{1000}}D_{KL}(\mathcal{Q}||\mathcal{Q}_{1:4})$.
We can see that for this setting our gain is twofold:
\begin{enumerate}
	\item The denominator of the KL-divergence term goes to zero as the number of data points goes to infinity.
	\item The KL-divergence itself is smaller, since the hyper-posterior $\mathcal{Q}$ is quite close to the learned hyper-prior $\mathcal{Q}_{1:4}$.
\end{enumerate}

\LF{I also tried adaptive $\lambda$, but the result was a slightly slower ERM, which makes sense since the learning problem is easy}
...
\begin{proposition}
	...
	where \RM{You use both $D_T$ and ${\cal D}_T$. Are they the same?} \LF{Fixed, it's a mistake, it should be ${\cal D}_T$}
...
\begin{proof}
	...
	Define $f\triangleq F\circ A$, and let 
	$$\xi((\mathcal{P}_S,P_S), \mathcal{D}, f)=\int_{\mathcal{Z}^m}\int_{\mathcal{M}(\mathcal{H})\times\mathcal{H}}e^{f(S, h)}\mathcal{P}_S(dP)P'_{S,P}(dh)\mathcal{D}(dS)$$
	...
	Using Equation \ref{eq:ribasplata-pb-appendix} with $f(S,(P,h))=\lambda(\mathcal{L}(h,\mathcal{D})-\hat{\mathcal{L}}(h,S))$, a 2-level posterior $(\mathcal{Q}_S, Q_S)$ \RM{You use $Q_S$ on the left and right of the definition - this is confusing} \LF{This should be better, the left side is unnecessary. } and a 2-level prior $(\mathcal{Q}_{1:N}, Q_S)$ ,
	...
	For any $\delta_T \in (0,1)$, the following holds uniformly for all hyper-posteriors $\mathcal{Q}_{S_T}\in \mathcal{M}(\mathcal{M}(\mathcal{H}))$ \RM{Should be ${\cal Q}_{S_T}$} \LF{Fixed} with probability at least $1-\delta_T$ over the draw of $S_T\sim \mathcal{D}_T$:
	...
	+\frac{1}{\lambda}\log\frac{\tilde{\xi}(\lambda,\mathcal{Q}_{1:N},\mathcal{D}_T)}{\delta_T}
	\end{align*}
	\RM{The last term should be as in \eqref{eq:tilde_xi}} \LF{Fixed}
