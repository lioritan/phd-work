...
This moment-generating function intuitively quantifies the concentration of the function $f$ in the stochastic kernel, and will be low if $f$ is ell-concentrated.
...
\begin{proof}
	...
	Using Equation \ref{eq:ribasplata-pb-appendix} with $f(S,(P,h))=\lambda(\mathcal{L}(h,\mathcal{D})-\hat{\mathcal{L}}(h,S))$, $Q_S=(\mathcal{Q}_S, Q_S)$ and $P_S=(\mathcal{Q}_{1:N}, Q_S)$,
	...
	For any $\delta_T \in (0,1)$, the following holds uniformly for all hyper-posteriors $Q_{S_T}\in \mathcal{M}(\mathcal{M}(\mathcal{H}))$ with probability at least $1-\delta_T$ over the draw of $S_T\sim \mathcal{D}_T$:
