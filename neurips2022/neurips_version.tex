...
We note that $\xi(\lambda,\mathcal{Q}_{1:N})$ has a dual expectation and $Q(P,S_T)$ depends on $S_T$, meaning classic bounds on the log-moment (such as Hoeffding's Lemma) cannot be used here. Using the product property of the KL-divergence (as in Equation \ref{eq:kl-decompose}), we can write this inequality as:
