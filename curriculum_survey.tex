...
\subsection{(Ongoing) Task adaptation and PB - 22.11.21} \label{subsec:pb-adapt}

\subsubsection{Option 1 - take DA bound and apply PB on source}
Take bound of the form $$R_{\mathcal{T}}(h)\leq R_{\mathcal{S}}(h)+C\sqrt{JS(\mathcal{T}||\mathcal{S})}$$

Take expectation over $h\sim Q$, apply PB bound on $E_{h\sim Q}\left [R_{\mathcal{S}}(h)\right ]$

\subsubsection{Option 2 - take PB bound and use it on a DA expression}
Take a bound of form $$F(\mathcal{L}, H, S, Q)\leq KL(Q||P) + log\frac{\xi}{\delta}$$

and choose a pair hypothesis $Q=(Q,Q_0), P=(Q_0, P_0)$ and $$F=\lambda_1 \left (\mathcal{L}(Q, \mathcal{T})-\hat{\mathcal{L}}(Q,T)\right )-\lambda_2 \left (\mathcal{L}(Q_0,\mathcal{S})+\hat{\mathcal{L}}(Q_0,S) \right )$$

Or something very similar, and possibly use the fact that KL-divergence does not obey triangle inequality to gain something over $KL(Q||P_0)$.

\subsubsection{Option 3 - Joint CoM inequality}
Option 1 is derived via CoM on the data distributions $\mathcal{T},\mathcal{S}$.
Option 2 is derived via CoM on the hypothesis distributions $Q,P$.

Can a CoM from $\mathcal{S},P$ to $\mathcal{T},Q$ give a useful term?
