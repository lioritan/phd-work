...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Math for PAC-Bayesian meta learning - 8.7.21} \label{sec:bayes}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
To minimize this, we would like to balance minimizing $E_{P\sim \mathcal{Q}}\hat{\mathcal{L}}(Q(P, S_i), S_i)$ and $KL(\mathcal{Q}||\mathcal{P})$. TODO: what can we say about this? it may be optimal to give hypothesis weights based on distance to $D_i$, but how can this be proven, and does this use data from $\{S_j\}$?
Ignoring the data from other tasks and picking $\mathcal{Q}$ that is a gibbs distribution on $\mathcal{P},S_i$ gives us $KL(\mathcal{Q}||\mathcal{P})\propto E_{P\sim \mathcal{P}}ln Z_\beta(S_i,P)$, where $Z_\beta(S_i,P)=\int_H P(h)e^{\beta \hat{\mathcal{L}}(h, S_i)}dh$.

\subsubsection*{Approach 2}
Let us try to bound
$$E_{P\sim \mathcal{Q}}\bigl [E_{S\sim D_i^{m_i}}\mathcal{L}(Q(P, S), D_i)-\frac{1}{n}\Sigma_k\hat{\mathcal{L}}(Q(P, S_k), S_i)\bigr ]$$

We basically get the same thing with mean loss of learners trained on any dataset and tested on $S_i$. 
The moment term should be smaller (i.e. the bound is tighter) since we can expect $\hat{\mathcal{L}}(Q(P, S_j), S_i)\geq \hat{\mathcal{L}}(Q(P, S_i), S_i) $ for any prior (this may not be the case, but it makes sense). TODO: it should be optimal to give meta-hypothesis weights based on $KL(Q_j||Q_i)$, but how can this be proven? Is this bound better?
