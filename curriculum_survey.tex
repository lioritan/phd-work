...
\subsubsection{Using meta-learners as priors - 12.10.21}
...
\mathcal{Q}_{gibbs}(S_t)(P)  &\text{w.p. } 1-\alpha
\end{cases} $$

Where $\mathcal{Q}_0$ is a data-free meta-learner, $\alpha$ is not a function of $P$ and $\mathcal{Q}_{gibbs}(S_t)$ is the gibbs meta-learner for $S_t$ with meta-prior $\mathcal{P}$.
We know from \cite{Rivasplata2020} that with probability at least $1-\delta_1$ over the choice of $S_t$:

\begin{equation*}
\begin{split}
E_{P\sim \mathcal{Q}_\alpha} \left [\mathcal{L}(Q_t, D_t)\right ] \leq  E_{P\sim \mathcal{Q}_\alpha} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] + \frac{1}{\sqrt{m}}KL(\mathcal{Q}_\alpha||\mathcal{Q}_{gibbs}(S_t)) + \frac{1}{\sqrt{m}}C_1(a,b, \delta_1)
\end{split}
\end{equation*}
...
\begin{equation*}
\begin{split}
E_{P\sim \mathcal{Q}_\alpha} \left [\mathcal{L}(Q_t, D_t)\right ] \leq  E_{P\sim \mathcal{Q}_\alpha} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] + \frac{1}{\sqrt{m}}\min\left \{KL(\mathcal{Q}_\alpha||\mathcal{Q}_{gibbs}(S_t))+C_1(a,b, \delta_1), KL(\mathcal{Q}_\alpha||\mathcal{Q}_0)+C_2(a,b,\delta_2)\right \} 
\end{split}
\end{equation*}

Using the smoothing property, if $\alpha$ does not depend on $P$, we can derive that 

$$ E_{P\sim \mathcal{Q}_\alpha} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] = E_{\alpha}E_{P\sim \mathcal{Q}_a} \left [\hat{\mathcal{L}}(Q_t, S_t)|\alpha=a \right ] = \alpha E_{P\sim \mathcal{Q}_0} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] + (1-\alpha)E_{P\sim \mathcal{Q}_{gibbs}(S_t)} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] $$

And similarly for the KL-divergences

$$ KL(\mathcal{Q}_\alpha||\mathcal{Q}_{gibbs}(S_t)) = \alpha KL(\mathcal{Q}_0||\mathcal{Q}_{gibbs}(S_t)) + (1-\alpha)KL(\mathcal{Q}_{gibbs}(S_t)||\mathcal{Q}_{gibbs}(S_t)) = \alpha KL(\mathcal{Q}_0||\mathcal{Q}_{gibbs}(S_t))$$

$$ KL(\mathcal{Q}_\alpha||\mathcal{Q}_0) = \alpha KL(\mathcal{Q}_0||\mathcal{Q}_0) + (1-\alpha)KL(\mathcal{Q}_{gibbs}(S_t)||\mathcal{Q}_0) = (1-\alpha)KL(\mathcal{Q}_{gibbs}(S_t)||\mathcal{Q}_0)$$

Meaning 

\begin{equation*}
\begin{split}
E_{P\sim \mathcal{Q}_\alpha} \left [\mathcal{L}(Q_t, D_t)\right ] &\leq  \alpha E_{P\sim \mathcal{Q}_0} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] + (1-\alpha)E_{P\sim \mathcal{Q}_{gibbs}(S_t)} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ]  \\ &+ \frac{1}{\sqrt{m}}\min\left \{\alpha KL(\mathcal{Q}_0||\mathcal{Q}_{gibbs}(S_t))+C_1(a,b, \delta_1), (1-\alpha)KL(\mathcal{Q}_{gibbs}(S_t)||\mathcal{Q}_0)+C_2(a,b,\delta_2)\right \} 
\end{split}
\end{equation*}

For large $m$, $C_1$ and $C_2$ are negligible, and therefore if $E_{P\sim \mathcal{Q}_0} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] < E_{P\sim \mathcal{Q}_{gibbs}(S_t)} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ]$, the choice of $\alpha=1$ is optimal, and otherwise the choice of $\alpha=0$ is optimal (if both give the same loss, pick one).
...
(1-\alpha) \mathcal{Q}_{gibbs}(S_t)(P) $$ 

Where as before, $\alpha$ is not a function of $P$, $\mathcal{Q}_0$ is some data-free hyper-posterior, and $\mathcal{Q}_{gibbs}(S_t)$ is the gibbs meta-learner.

As before, we can choose $\delta_1=\frac{\delta}{2}, delta_2=\frac{\delta}{2}$ such that with probability at least $1-\delta$,

\begin{equation*}
\begin{split}
E_{P\sim \mathcal{Q}'_\alpha} \left [\mathcal{L}(Q_t, D_t)\right ] \leq  E_{P\sim \mathcal{Q}'_\alpha} \left [\hat{\mathcal{L}}(Q_t, S_t)\right ] + \frac{1}{\sqrt{m}}\min\left \{KL(\mathcal{Q}'_\alpha||\mathcal{Q}_{gibbs}(S_t))+C_1(a,b, \delta), KL(\mathcal{Q}'_\alpha||\mathcal{Q}_0)+C_2(a,b,\delta)\right \} 
\end{split}
\end{equation*}

$$ KL(\mathcal{Q}'_\alpha||\mathcal{Q}_0) = \int \mathcal{Q}'_\alpha ln\frac{\mathcal{Q}'_\alpha}{\mathcal{Q}_0} dP = \int \mathcal{Q}'_\alpha ln\left (\alpha + \frac{(1-\alpha)\mathcal{Q}_{gibbs}(S_t)}{\mathcal{Q}_0}\right ) dP$$

Since $\alpha\leq 1$ and the second term within the logarithm is non-negative, we can use the identity $ln(1+x)\leq x$ to derive:

\begin{equation*}
\begin{split}
KL(\mathcal{Q}'_\alpha||\mathcal{Q}_0) & \leq \int \mathcal{Q}'_\alpha \frac{(1-\alpha)\mathcal{Q}_{gibbs}(S_t)}{\mathcal{Q}_0} dP \\ &= \int \alpha(1-\alpha)\mathcal{Q}_{gibbs}(S_t)+ \frac{(1-\alpha)^2\mathcal{Q}_{gibbs}(S_t)^2}{\mathcal{Q}_0}  dP \\& = \alpha(1-\alpha)\int \mathcal{Q}_{gibbs}(S_t) dP + (1-\alpha)^2\int \frac{\mathcal{Q}_{gibbs}(S_t)^2}{\mathcal{Q}_0}  dP \\ &= \alpha(1-\alpha) + (1-\alpha)^2\int \frac{\mathcal{Q}_{gibbs}(S_t)^2}{\mathcal{Q}_0}  dP
\end{split}
\end{equation*}

And similarly we can derive 

$$ KL(\mathcal{Q}'_\alpha||\mathcal{Q}_{gibbs}(S_t)) \leq \alpha(1-\alpha) + \alpha^2\int \frac{\mathcal{Q}_0^2}{\mathcal{Q}_{gibbs}(S_t)}  dP$$

And using these, we can deduce

\begin{equation}
\begin{split}
\mathcal{L}(\mathcal{Q}'_\alpha, D_t) \leq  \hat{\mathcal{L}}(\mathcal{Q}'_\alpha, S_t) + \frac{1}{\sqrt{m}}\min\left \{\alpha^2\int \frac{\mathcal{Q}_0^2}{\mathcal{Q}_{gibbs}(S_t)}  dP, (1-\alpha)^2\int \frac{\mathcal{Q}_{gibbs}(S_t)^2}{\mathcal{Q}_0}  dP\right \} + \frac{1}{\sqrt{m}}C(a,b,\delta, \alpha)
\end{split}
\end{equation}
...
Let us consider the case where $\alpha=\alpha(P)$, meaning that the combination depends on the choice of prior.
