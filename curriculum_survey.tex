...
\subsubsection*{Approach 3 - 9.8.21} \label{section-meta-known}
...
\\ 
+\textcolor{olive} {\frac{1}{n^2\beta}\sum_k E_{P\sim \mathcal{Q}} \left [C(S_i,S_k,P)\right ]}
\end{split}
\end{equation*}
...
\subsection{Task dependencies with multiple tasks - 9.8.21}
...
\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}_i, D_t)\leq \textcolor{blue}{\frac{1}{n}\sum_k E_{P\sim \mathcal{Q}_i} \left [ \hat{\mathcal{L}}(Q_t,S_k) -\frac{1}{n\beta}KL(Q_t||Q_k) \right ]} + \textcolor{red}{\frac{1}{n\beta} KL(\mathcal{Q}_i||\mathcal{P}) +\frac{1}{n\beta}E_{P\sim \mathcal{Q}_i} KL(Q_i||P)} \\+ \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{nM} \right ]
\\ 
+\textcolor{olive} {\frac{1}{n^2\beta}\sum_k E_{P\sim \mathcal{Q}_i} \left [C(S_t,S_k,P)\right ]}
\end{split}
\end{equation*}

If we do not find the correct hyper-posterior, then there is a task $j\neq i$ such that $D_j\neq D_t$ but $E_{P\sim\mathcal{Q}_j}\left [\hat{\mathcal{L}}(Q(P,S_t), S_t)\right ]\leq E_{P\sim\mathcal{Q}_i}\left [\hat{\mathcal{L}}(Q(P,S_t), S_t)\right ]$. We get:

\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}_j, D_t)\leq \hat{\mathcal{L}}(\mathcal{Q}_j, S_t) + \frac{1}{n\beta} KL(\mathcal{Q}_j||\mathcal{P}) +\frac{1}{n\beta}E_{P\sim \mathcal{Q}_j} KL(Q_j||P) \\+ \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{nM} \right ]
\end{split}
\end{equation*}

And since $\hat{\mathcal{L}}(\mathcal{Q}_j, S_t)\leq \hat{\mathcal{L}}(\mathcal{Q}_i, S_t)$, we can replace and get 

\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}_j, D_t)\leq \textcolor{blue}{\frac{1}{n}\sum_k E_{P\sim \mathcal{Q}_i} \left [ \hat{\mathcal{L}}(Q_t,S_k) -\frac{1}{n\beta}KL(Q_t||Q_k) \right ]} + \textcolor{red}{\frac{1}{n\beta} KL(\mathcal{Q}_j||\mathcal{P}) +\frac{1}{n\beta}E_{P\sim \mathcal{Q}_j} KL(Q_j||P)} \\+ \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{nM} \right ]
\\ 
+\textcolor{olive} {\frac{1}{n^2\beta}\sum_k E_{P\sim \mathcal{Q}_i} \left [C(S_t,S_k,P)\right ]}
\end{split}
\end{equation*}
...
\subsubsection*{Bounding the mistake gap - 10.8.21}

To give a clearer expression to this KL term, we should consider the following:

\begin{equation*}
\begin{split}
\mathcal{Q}^{*}_{i}(P)=\frac{\mathcal{P}(P) e^{-\lambda g_i(P)}}{E_{P\sim \mathcal{P}} \left [ e^{-\lambda g_i(P) } \right ]} \\
g_i(P)=\hat{\mathcal{L}}(Q_i,S_i)+ \frac{1}{n\beta}KL(Q_i||P)
\end{split}
\end{equation*}
...
\subsubsection*{Comparison with Rothfuss bound - 10.8.21}

Now, let us try and compare our bound with the generalization bound from Rothfuss.

Using 
$$g(P)=\frac{1}{n}\sum_k \left ( \hat{\mathcal{L}}(Q_i,S_k) + \frac{1}{n\beta} C(S_i,S_k,P) -   \frac{1}{n\beta}  KL(Q_i||Q_k) \right )+ \frac{1}{n\beta}KL(Q_i||P)$$
The new multi-task bound is:

\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}, D_t)\leq \textcolor{blue}{E_{P\sim \mathcal{Q}_i} \left [ g(P) \right ]} + \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{nM} \right ]\\
+ \textcolor{red}{\frac{1}{n\beta} KL(\mathcal{Q}_i||\mathcal{P}) + \frac{1}{n\beta}\Psi(\mathcal{Q},\mathcal{Q}_i,\lambda)  } \\
\end{split}
\end{equation*}
...
We note that the per-task kl term is folded into $g(P)$.
The Rothfuss bound is:
...
+ \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{M+K} \right ]
\\+
\textcolor{red}{(\frac{1}{\lambda}+ \frac{1}{n\beta}) KL(\mathcal{Q}||\mathcal{P}) + \frac{1}{n\beta}\sum_k \mathbb{E}_{P\sim \mathcal{Q}} \left [KL(Q_k||P) \right ]}\\ +\frac{1}{\lambda}ln \mathbb{E}_{P\sim \mathcal{P}} [e^{\frac{\lambda}{n}\sum_{k}\mathbb{E}_{\mathcal{T}}[\mathcal{L}(Q(P,S),D)- \mathcal{L}(Q_k, \mathcal{D}_k)]}]
\end{split}
\end{equation*}
