...
\subsection{Using the adaptation data - 24.2.22}
...
Using \cite{Rivasplata2020} on a 2-level hypothesis $(\mathcal{Q},Q), (\mathcal{P}, P)$, with $\mathcal{P}=\mathcal{Q}_{1:N}$, we get 
...
By simplifying the third and fourth terms using Jensen's inequality (this is not optimal but sufficient for now) we get:

\begin{equation} \label{eq:pb-adapt-multi}
\mathcal{L}(\mathcal{Q}_{1:N,T}, D_T) \leq 
(1-\frac{\beta}{\lambda})\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T) +(\frac{\beta-\gamma}{\lambda})\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) + \frac{\gamma}{\lambda}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T) +\frac{1}{\lambda}ln\frac{\xi(\lambda,\mathcal{Q}_{1:N})}{\delta_t}
\end{equation}

Deriving by $\gamma$ gets us:

$$(1-\frac{\beta}{\lambda})\frac{d}{d\gamma}\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T) -\frac{1}{\lambda}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) + (\frac{\beta-\gamma}{\lambda})\frac{d}{d\gamma}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) +\frac{1}{\lambda}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T) =0$$
...
\begin{align*} 
\begin{split}
&-(\lambda-\beta)\left (E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)\hat{\mathcal{L}}(Q(P,S_T),S_T)\right]+R(\gamma)\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)\right )\\& - (\beta-\gamma)\left (E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)^2\right]+R(\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)\right )\\& =\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)
\end{split}
\end{align*}

\begin{align*} 
\begin{split}
 -(\beta-\gamma) = \frac{\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)+(\lambda-\beta)\left (E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)\hat{\mathcal{L}}(Q(P,S_T),S_T)\right]+R(\gamma)\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)\right )}{E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)^2\right]+R(\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)}
\end{split}
\end{align*}

\begin{align} 
\begin{split}
\gamma = \beta+\frac{\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)+(\lambda-\beta)\left (E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)\hat{\mathcal{L}}(Q(P,S_T),S_T)\right]+R(\gamma)\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)\right )}{E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)^2\right]+R(\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)}
\end{split}
\end{align}

We note that if $\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)$ is low, we should have $\lambda < \beta$ (and we will later see this seems likely) to ensure $\gamma < \beta$. If $\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)$ is high or the gain from using $S_T$ for the hyper-posterior is high, unless $\lambda \gg \beta$ (unlikely) we have $\gamma < \beta$. This means that the implicit function encourages a stable equilibrium (lower $\gamma$) if there is a benefit to using $S_T$ in the hyper-posterior, and encourages increasing $\gamma$ if the base hyper-posterior is already good.

Deriving Equation \ref{eq:pb-adapt-multi} by $\beta$ gives us (since $\hat{\mathcal{L}}'$ does not depend on $\beta$, and the moment is on $P$):

$$-\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)+(\lambda-\beta)\frac{d}{d\beta}\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)+\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)=0 $$

In a similar manner to before, we get

$$-(\lambda-\beta)E_{P\sim \mathcal{Q}_{1:N,T}}E_{h\sim Q(P,S_T)}\left [\hat{\mathcal{L}}(h,S_T)^2+R'(\beta)\hat{\mathcal{L}}(h, S_T) \right ] =\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) $$
...
\begin{align} 
\begin{split}
\beta = \lambda+\frac{\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)}{E_{P\sim \mathcal{Q}_{1:N,T}}E_{h\sim Q(P,S_T)}\left [\hat{\mathcal{L}}(h,S_T)^2+R'(\beta)\hat{\mathcal{L}}(h, S_T) \right ]}
\end{split}
\end{align}
...
Plugging this value into our equation for $\gamma$, if we assume all of the gibbs derivative terms are equal to some constant $C$ (this is not a reasonable assumption, but it makes the calculation much easier) gives us:

\begin{align} 
\begin{split}
\gamma &= \lambda+\frac{\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)}{C}+\frac{\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)+(-\frac{\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)}{C})C}{C}\\
&=\lambda+\frac{\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)}{C}
\end{split}
\end{align}
...
$$\beta \hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-(\beta-\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\gamma\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T)-ln\frac{\xi(\mathcal{Q}_{1:N}\lambda)}{\delta_t}+\lambda\frac{d}{d\lambda}ln\xi(\mathcal{Q}_{1:N}\lambda)=0$$

$$\beta(\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T))+\gamma(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T))-ln\frac{\xi(\mathcal{Q}_{1:N}\lambda)}{\delta_t}=\lambda\frac{d}{d\lambda}ln\xi(\mathcal{Q}_{1:N}\lambda)=0$$
...
\begin{align} 
\begin{split}
\lambda = \frac{\beta(\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T))+\gamma(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T))-ln\frac{\xi(\mathcal{Q}_{1:N}\lambda)}{\delta_t}}{R_\xi(\lambda)}
\end{split}
\end{align}

This is a rather interesting expression, as we can see that if the transfer from $\mathcal{Q}_{1:N}$ to $\mathcal{Q}_{1:N,T}$ is beneficial, we increase $\lambda$ in proportion to $\gamma$ times the relative gain, and similarly for $\beta$ and the gain from the base learner. The moment term acts as a counter-balance, as increasing $\lambda$ too much will cause the moment to diverge. This means that the more we gain from a specific type of adaptation via $S_T$, the more weight we should give to the empirical loss part of the bound (compared to the complexity part). One odd thing about this equation is that we can expect $R_\xi(\lambda)<0$, making these observations strange.
