...
\subsubsection*{Approach 3 - 30.7.21}
...
$$ \hat{\mathcal{L}}(Q_i,S_i)\leq \hat{\mathcal{L}}(Q_i,S_k) - \frac{1}{\gamma} KL(Q_i||Q_k) + \frac{1}{\gamma}C(S_i,S_k,P)$$
...
$$ \mathcal{L}(\mathcal{Q}, D_i)\leq E_{P\sim \mathcal{Q}} \left [ \frac{1}{n}\sum_k\hat{\mathcal{L}}(Q_i,S_k) - \frac{1}{n\gamma} \sum_k KL(Q_i||Q_k) + \frac{1}{n\gamma}\sum_k C(S_i,S_k,P) \right ]+ KL(\mathcal{Q}||\mathcal{P})+\Upsilon^{\text{\Romannum{2}}}(\mathcal{P}, S_i)$$

Let's mark:
$$g(P)=\frac{1}{n}\sum_k \left ( \hat{\mathcal{L}}(Q_i,S_k) + \frac{1}{\gamma} C(S_i,S_k,P) -   \frac{1}{\gamma}  KL(Q_i||Q_k) \right )$$
...
\subsubsection*{Footnote : developing $C(S_i,S_k,P)$ - 28.7.21}
...
Since we know that for any random variables $E(XY)=E(X)E(Y)+cov(X,Y)\rightarrow E(X)E(Y)\leq E(XY)$, so:

$$\frac{1}{n\gamma}\sum_k C(S_i,S_k,P) \leq \frac{1}{n\gamma}ln E_{h\sim P} \Pi_k \left [e^{-\gamma\hat{\mathcal{L}}(h,S_k)}\right ] - \frac{1}{\gamma}lnZ_{\gamma}(P,S_i)
= \frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{\mathcal{L}}(h,S_k)}\right ] - \frac{1}{\gamma}lnZ_{\gamma}(P,S_i)$$

From Jensen's inequality, we know that $E(ln(X))\leq ln(E(X))$, so we can get:

\begin{equation*}
\begin{split}
\frac{1}{n\gamma}\sum_k C(S_i,S_k,P) \leq \frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{\mathcal{L}}(h,S_k)}\right ] - \frac{1}{\gamma}ln E_{h\sim P}\left [e^{-\gamma\hat{\mathcal{L}}(h,S_i)}\right ] \\
\leq \frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{L}(h,S_k)}\right ] - \frac{1}{\gamma} E_{h\sim P}ln\left [e^{-\gamma \hat{\mathcal{L}}(h,S_i)}\right ]  \\
=\frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{\mathcal{L}}(h,S_k)}\right ] - \frac{1}{\gamma} E_{h\sim P}\left [-\gamma \hat{\mathcal{L}}(h,S_i)\right ] \\
=\frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{\mathcal{L}}(h,S_k)}\right ] + E_{h\sim P}\left [ \hat{\mathcal{L}}(h,S_i)\right ]
\end{split}
\end{equation*}

Putting it together, we get:

$$\frac{1}{n\gamma}\sum_k C(S_i,S_k,P) \leq \frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{L}(h,S_k)}\right ] + E_{h\sim P}\left [ \hat{\mathcal{L}}(h,S_i)\right ]$$
...
$$\frac{1}{n\gamma}\sum_k C(S_i,S_k,P) \leq \frac{1}{n\gamma}ln E_{h\sim P} \left [e^{-\gamma\sum_k 0}\right ] + E_{h\sim P}\left [ \hat{\mathcal{L}}(h,S_i)\right ]=E_{h\sim P}\left [ \hat{\mathcal{L}}(h,S_i)\right ]$$

We also note that we can also minimize for 
$$\bar{g}(P)=\frac{1}{n}\sum_k \left ( \hat{\mathcal{L}}(Q_i,S_k)  -  \frac{1}{\gamma}  KL(Q_i||Q_k) + E_{h\sim P}\hat{\mathcal{L}}(h,S_i) \right ) + \frac{1}{\gamma} ln E_{h\sim P} \left [e^{-\gamma\sum_k \hat{\mathcal{L}}(h,S_k)}\right ] $$ 

\subsubsection*{Conclusions from approach 3 - 28.7.21}

Let us compare this bound to Rothfuss:

$$\mathcal{L}(\mathcal{Q}, \mathcal{T}) \leq \hat{\mathcal{L}}(\mathcal{Q}, S_1,...,S_n) + (\frac{1}{\lambda}+\frac{1}{n\beta}) KL(\mathcal{Q}||\mathcal{P}) + \frac{1}{n\beta}\sum_i \mathbb{E}_{P\sim \mathcal{Q}} \left [KL(Q_i||P) \right ] + \frac{1}{n\beta}ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{\beta\sum_i\mathcal{L}(h,\mathcal{D}_i)- \hat{\mathcal{L}}(h, S_i)} \right ] +\Upsilon^{\text{\Romannum{2}}}(\lambda)$$

$$ \mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_i,S_k) \right ] +  KL(\mathcal{Q}||\mathcal{P}) - \frac{1}{n\gamma} \sum_k E_{P\sim \mathcal{Q}} \left [ KL(Q_i||Q_k) \right ] + ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{\frac{1}{n}\sum_i\mathcal{L}(h,\mathcal{D}_i)- \hat{\mathcal{L}}(h, S_i)} \right ]
...
$$ \mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_i,S_k) \right ] +  \frac{1}{n\beta} KL(\mathcal{Q}||\mathcal{P}) - \frac{1}{n^2\beta} \sum_k E_{P\sim \mathcal{Q}} \left [ KL(Q_i||Q_k) \right ] + \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{\beta\sum_i\mathcal{L}(h,\mathcal{D}_i)- \hat{\mathcal{L}}(h, S_i)} \right ]
+ \Upsilon^{\text{\Romannum{1}}}(n\beta) $$

After we mark the now common moment term as $M$, we have:

\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}, \mathcal{T}) \leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_k,S_k) \right ] + (\frac{1}{\lambda}+\frac{1}{n\beta}) KL(\mathcal{Q}||\mathcal{P}) + \frac{1}{n\beta}\sum_k \mathbb{E}_{P\sim \mathcal{Q}} \left [KL(Q_k||P) \right ]\\ + M +\frac{1}{\lambda}ln \mathbb{E}_{P\sim \mathcal{P}} [e^{\frac{\lambda}{n}\sum_{k}\mathbb{E}_{\mathcal{T}}[\mathcal{L}(Q(P,S),D)- \mathcal{L}(Q_k, \mathcal{D}_k)]}]
\end{split}
\end{equation*}

\begin{equation*}
\begin{split}
\mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_i,S_k) \right ] +  \frac{1}{n\beta} KL(\mathcal{Q}||\mathcal{P}) - \frac{1}{n^2\beta} \Sigma_k E_{P\sim \mathcal{Q}} \left [ KL(Q_i||Q_k) \right ] \\+ M 
...
$\mathcal{L}(\mathcal{Q}, \mathcal{T}) \leq  E_{P\sim \mathcal{Q}}  \left [ \hat{\mathcal{L}}(Q_i,S_i) \right ] + (1+\frac{1}{m})KL(\mathcal{Q}||\mathcal{P}) + \frac{1}{m}\mathbb{E}_{P\sim \mathcal{Q}} \left [KL(Q_k||P)\right ] + M + \Upsilon^{\text{\Romannum{2}}}(n)$

Compared to 

$\mathcal{L}(\mathcal{Q}, D_i)\leq E_{P\sim \mathcal{Q}}  \left [ \hat{\mathcal{L}}(Q_i,S_i) \right ] + \frac{1}{m}KL(\mathcal{Q}||\mathcal{P}) + M $
...
TODO: understand why $\frac{KL(Q||P)}{\sqrt{m}}\rightarrow 0$ as $m$ goes to $\infty$ (and how it applies).
For consistent bounds we need to pick $\lambda=\sqrt{n}, \beta=\sqrt{m}$

If $n\rightarrow\infty$, we get 

$\mathcal{L}(\mathcal{Q}, \mathcal{T}) \leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_k,S_k) \right ] + \frac{1}{n\sqrt{m}}\sum_k \mathbb{E}_{P\sim \mathcal{Q}} \left [KL(Q_k||P) \right ] + M$

Compared to 

$\mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}}  \left [ \hat{\mathcal{L}}(Q_i,S_k) \right ]  + M + \frac{1}{n^2\sqrt{m}}\sum_k E_{P\sim \mathcal{Q}} \left [C(S_i,S_k,P)\right ] $

where $\frac{1}{n^2\sqrt{m}}\sum_k E_{P\sim \mathcal{Q}} \left [C(S_i,S_k,P)\right ] \leq  \mathcal{L}(\mathcal{Q}, S_i)$ for non-negative loss functions. 
This may be a tighter bound if there are many tasks that are similar (in terms of loss of trained classifiers) to task i.

If $m\rightarrow\infty$, we get

$\mathcal{L}(\mathcal{Q}, \mathcal{T}) \leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}} \left [ \hat{\mathcal{L}}(Q_k,S_k) \right ]+ \frac{1}{n}KL(\mathcal{Q}||\mathcal{P}) + \Upsilon^{\text{\Romannum{2}}}(\sqrt{n})$

Compared to 

$\mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}}  \left [ \hat{\mathcal{L}}(Q_i,S_k) \right ] + \frac{1}{n\sqrt{m}}KL(\mathcal{Q}||\mathcal{P}) + \frac{1}{n^2\sqrt{m}}\sum_k E_{P\sim \mathcal{Q}} \left [C(S_i,S_k,P)\right ] $
