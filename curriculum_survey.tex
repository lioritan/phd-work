...
\LF{Made the transition clearer and explicit}
...
\begin{defn}
	...
	We define $$\hat{\mathcal{L}}_{meta}\triangleq \hat{\mathcal{L}}'^{(\gamma)}(\mathcal{Q}_{1:N,T}, S_T)=E_{P\sim \mathcal{Q}^{\gamma}_{1:N,T}}E_{h\sim P}\left [\hat{\mathcal{L}}(h, S_T)\right ]$$ as the meta adapted loss, meaning $S_T$ is used only on the hyper-prior level. $\hat{\mathcal{L}}_{meta}$ is a function of $\gamma$.
...
\end{defn}
                             \RM{Present all definitions in display mode. I would add an index $\beta,\gamma$ to clarify what parameters they depend on.}
                             
                             \LF{Changed to display mode, added indices to the definitions.}
...
We used SGLD \LF{Need to add citation to Welling and Teh 2011} to approximate the optimal empirical Gibbs posterior.
In order to get unbiased loss estimates, we used subsampling and query/evaluate sets for meta-adaptation \LF{If this is not done, is there any true difference between meta-adaptation and base adaptation?}
