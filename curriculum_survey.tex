...
\subsection{Task adaptation and PB - 6.12.21} \label{subsec:pb-adapt}
...
If $Q\propto Q_0 e^{-\beta \hat{\mathcal{L}}(h,T)}$, we have $(2)<0$ if:
$$ -log Z_{\beta}(P, T) > -log Z_{\beta}(Q_0, T) +KL(Q_0||P)$$

In other words, 

$$KL(Q_0||P) \leq log\frac{E_{h\sim Q_0}\left [e^{-\beta\hat{\mathcal{L}}(h,T)}\right ]}{E_{h\sim P}\left [e^{-\beta\hat{\mathcal{L}}(h,T)}\right ]}$$

\textcolor{red}{The best source-target transfer is if the source has good performance on target without being too far in KL, and also we learn the source task reasonably well. This suggests that we should try to combine tasks with high test gain constrained by not too high KL. Unlike option 1, we need labeled test data for this to work.}

\subsection{Using the adaptation data - 19.1.22}
...
Using \cite{Rivasplata2020}, with $\mathcal{P}=\mathcal{Q}_{1:N}$, we get (from the fact that the base learner is gibbs with parameter $\beta$):

$$\mathcal{L}(\mathcal{Q}_{1:N,T}, D_T) \leq (1-\frac{\beta}{\lambda})\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T) + \frac{1}{\gamma}KL(\mathcal{Q}_{1:N,T}||\mathcal{Q}_{1:N})-\frac{1}{\lambda}E_{P\sim \mathcal{Q}_{1:N,T}}\left [lnZ_{\beta}(S_T,P)\right ]+\frac{1}{\lambda}ln\frac{\xi(\lambda)}{\delta_t}$$
...
$$\mathcal{L}(\mathcal{Q}_{1:N,T}, D_T) \leq  -\frac{\gamma}{\beta}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) + \frac{\gamma}{\beta}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)-\frac{1}{\beta}E_{P\sim \mathcal{Q}_{1:N,T}}\left [lnZ_{\beta}(S_T,P)\right ]+\frac{1}{\beta}ln\frac{\xi(\beta)}{\delta_t}$$

We can also use Jensen's inequality on the third term here to get:

$$\mathcal{L}(\mathcal{Q}_{1:N,T}, D_T) \leq  -\frac{\gamma}{\beta}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) + \frac{\gamma}{\beta}\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)+\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T},S_T)+\frac{1}{\beta}ln\frac{\xi(\beta)}{\delta_t}$$
...
Using the rules for variance, we get:

$$(\beta-\gamma)\left (Var(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T},S_T))+\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)^2 +R(\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)\right )  =  \hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T},S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)$$

Assuming we consider the expectations to be constant, we can extract $\gamma$ (this results in a two-step optimization process of calculating $\gamma$ and updating the distribution):
...
Interestingly, we can expect the numerator to be negative, and if $\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)\approx 0$ we have $$\gamma\approx \beta-\frac{\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)}{ Var(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T},S_T))} $$

This suggests that if the adaptation is very good, the value of $\gamma$ should be lowered.
