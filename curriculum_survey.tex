...
\subsection{Using the adaptation data - 21.2.22}
...
We note that if $\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)$ is low, unless $\lambda < \beta$ we must have $\gamma > \beta$. If $\hat{\mathcal{L}}'(\mathcal{Q}_{1:N},S_T)$ is high, unless $\lambda \gg \beta$ we have $\gamma < \beta$.

Deriving Equation \ref{eq:pb-adapt-multi} by $\beta$ gives us:

$$-\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)+(\lambda-\beta)\frac{d}{d\beta}\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)+\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)+\frac{d}{d\beta}ln\xi(\mathcal{Q}_{1:N},\lambda)=0 $$

In a similar manner to before, we get

$$-(\lambda-\beta)E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)\hat{\mathcal{L}}(Q(P,S_T),S_T)+R'(\beta)\hat{\mathcal{L}}(Q(P,S_T), S_T) \right ] +\frac{d}{d\beta}ln\xi(\mathcal{Q}_{1:N},\lambda)=\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T) $$
...
\begin{align} 
\begin{split}
\beta = \lambda+\frac{\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\frac{d}{d\beta}ln\xi(\mathcal{Q}_{1:N},\lambda)}{E_{P\sim \mathcal{Q}_{1:N,T}}\left [\hat{\mathcal{L}}(P,S_T)\hat{\mathcal{L}}(Q(P,S_T),S_T)+R'(\beta)\hat{\mathcal{L}}(Q(P,S_T), S_T) \right ]}
\end{split}
\end{align}

Finally, deriving Equation \ref{eq:pb-adapt-multi} by $\lambda$ gets us:

$$\beta \hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-(\beta-\gamma)\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\gamma\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T)-ln\xi(\mathcal{Q}_{1:N},\lambda)+\lambda\frac{d}{d\lambda}ln\xi(\mathcal{Q}_{1:N}\lambda)=0$$

$$\beta(\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T))+\gamma(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T))-ln\xi(\mathcal{Q}_{1:N},\lambda)=\lambda\frac{d}{d\lambda}ln\xi(\mathcal{Q}_{1:N}\lambda)=0$$

Since $$\frac{d}{d\lambda}ln\xi(\mathcal{Q}_{1:N}\lambda)=\frac{E_{P\sim \mathcal{Q}_{1:N}}\left [-(\hat{L}(Q(P,S_T), S_T)-E_S\hat{L}(Q(P,S), S))e^{-\lambda(\hat{L}(Q(P,S_T), S_T)-E_S\hat{L}(Q(P,S), S))} \right ]}{E_{P\sim \mathcal{Q}_{1:N}}\left [e^{-\lambda(\hat{L}(Q(P,S_T), S_T)-E_S\hat{L}(Q(P,S), S))} \right ]}\triangleq R_\xi(\lambda)$$

We have

\begin{align} 
\begin{split}
\lambda = \frac{\beta(\hat{\mathcal{L}}(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T))+\gamma(\hat{\mathcal{L}}'(\mathcal{Q}_{1:N,T}, S_T)-\hat{\mathcal{L}}'(\mathcal{Q}_{1:N}, S_T))-ln\xi(\mathcal{Q}_{1:N},\lambda)}{R_\xi(\lambda)}
\end{split}
\end{align}

This is a rather interesting expression, as we can see that if the transfer from $\mathcal{Q}_{1:N}$ to $\mathcal{Q}_{1:N,T}$ is beneficial, we increase $\lambda$ in proportion to $\gamma$ times the relative gain, and similarly for $\beta$ and the gain from the base learner. The moment term acts as a counter-balance, as increasing $\lambda$ too much will cause the moment to diverge.
