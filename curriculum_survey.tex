...
\subsection{Task dependencies - 2.8.21}
...
\subsubsection*{Approach 3 - 2.8.21}
...
\subsubsection*{Footnote : developing $C(S_i,S_k,P)$ - 2.8.21}
...
\textcolor{red}{Since we know that for any random variables $E(XY)=E(X)E(Y)+cov(X,Y)\rightarrow E(X)E(Y)\leq E(XY)$:}
...
\textcolor{blue}{ We do note that if the loss is always at least zero, we can go further and get:}
...
$\mathcal{L}(\mathcal{Q}, D_i)\leq \frac{1}{n}\sum_k E_{P\sim \mathcal{Q}}  \left [ \hat{\mathcal{L}}(Q_i,S_k) -\frac{1}{n\sqrt{m}}KL(Q_i||Q_k)\right ]  + \frac{1}{n^2\sqrt{m}}\sum_k E_{P\sim \mathcal{Q}} \left [C(S_i,S_k,P)\right ] + \frac{1}{n\beta} ln\mathbb{E}_{P\sim \mathcal{P}}\mathbb{E}_{h\sim P}\left [ e^{nM} \right ]$
