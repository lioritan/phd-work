...
Using the result from \cite{Rothfuss2020}, by applying the change of measure inequality (see Seldin et. al \cite{}), for any $\lambda>0$,
...
For consistent bounds we need to pick $\lambda=\sqrt{n}, \beta=\sqrt{m}$. Assuming $KL(\mathcal{Q}||\mathcal{P})< \infty$, as $n\rightarrow \infty$,  $\frac{1}{\sqrt{n}}KL(\mathcal{Q}||\mathcal{P})\rightarrow 0$. For similar reasons $m\rightarrow \infty$ implies $\frac{1}{\sqrt{m}}KL(Q||P)\rightarrow 0$ for the per-task divergence.
