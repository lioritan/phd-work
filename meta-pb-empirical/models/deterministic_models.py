...
# Main function
# -------------------------------------------------------------------------------------------
def get_model(prm, input_shape, output_dim):
...
def batchnorm(input, weight=None, bias=None, running_mean=None, running_var=None, training=True, eps=1e-5, momentum=0.1):
    ...
    running_mean = torch.zeros(in_dim, device=input.get_device())
    running_var = torch.ones(in_dim, device=input.get_device())
...
#  OmConvNet
# -------------------------------------------------------------------------------- -----------
class OmConvNet_NoBN(base_model):
    def __init__(self, input_shape, output_dim):
