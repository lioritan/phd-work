...
def measure_difficulty(steps_per_task, tasks, wrapper, easy_task, student_alg="PPO"):
    ...
    random_teacher = PredefinedTasksTeacher({"tasks":
        [{"goal_angle":0}]*30 + [{"goal_angle": np.pi*0.9}, {"goal_angle": -np.pi*0.9}]*35
    }, wrapper)
    # random_teacher = PredefinedTasksTeacher({"tasks":
    #                                              #[{"goal_angle": np.pi*(1-1/i)} for i in range(100, 1, -1)]
    #                                              [{"goal_angle": np.pi * (1 / i)} for i in range(1, 101)]
    #                                              #[{"goal_angle": np.pi * (1 - i / 100)} for i in range(1, 101)]
    #                                          }, wrapper)
