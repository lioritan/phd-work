...
def load_expert_trajectory(folder_path, alp_thr=0.2, max_steps=10000000, is_toy_env=False):
    ...
    for i, (gmm_means, gmm_covs, episode) in enumerate(zip(data["means"], data["covariances"], data['episodes'])):
        ...
        if is_toy_env:
            ...
            all_rewards = np.interp(data['env_train_rewards'][episode:episode + gmm_step], (-150, 350),
                                    (0, 1))  # from gmm
...
class AgainTeacher(Teacher):
    def __init__(self, teacher_parameters, environment_parameters):
        ...
        self.classroom_filename = "student_history" if "classroom_filename" not in teacher_parameters else teacher_parameters[
            'classroom_filename']
        self.classroom_portion = 100 if "classroom_portion" not in teacher_parameters else teacher_parameters['classroom_portion']
        self.use_alpgmm = False if "use_alpgmm" not in teacher_parameters else teacher_parameters['use_alpgmm']
        self.pre_test_epoch_idx = 2 if "pretrain_epochs" not in teacher_parameters else teacher_parameters['pretrain_epochs']
        self.restart_after_pretrain = False if "restart_after_pretrain" not in teacher_parameters else teacher_parameters[
            'restart_after_pretrain']
        self.k = 5 if "k" not in teacher_parameters else teacher_parameters['k']
        self.random_expert = False if "random_expert" not in teacher_parameters else teacher_parameters['random_expert']
        self.nb_test_epochs = 0
        self.use_ground_truth = False if 'use_ground_truth' not in teacher_parameters else teacher_parameters['use_ground_truth']
        self.is_toy_env = False if "is_toy_env" not in teacher_parameters else teacher_parameters['is_toy_env']
        self.current_student_params = teacher_parameters['student_params']
        ...
        self.alpgmm = AlpGmmTeacher(teacher_parameters, environment_parameters)
        self.is_new_alpgmm = False  # boolean used to track alpgmm's periodic updates
        self.random_task_ratio = 0.1
        self.post_pre_test_task_ratio = 0.02 if "random_task_ratio" not in teacher_parameters else teacher_parameters["random_task_ratio"]
        self.in_end_rnd = self.post_pre_test_task_ratio if 'in_end_rnd' not in teacher_parameters else teacher_parameters['in_end_rnd']

        self.sampled_gaussian_idx = None

        self.stop_R = False if "stop_R" not in teacher_parameters else teacher_parameters['stop_R']
        ...
        self.expert_type = "P" if "expert_type" not in teacher_parameters else teacher_parameters["expert_type"]
        self.r_list_len = 50 if "r_list_len" not in teacher_parameters else teacher_parameters["r_list_len"]
        self.tol_ratio = 1.0 if "tol_ratio" not in teacher_parameters else teacher_parameters["tol_ratio"]
    ...
    def generate_task(self) -> Tuple[GymEnv, Dict[str, Any]]:
        ...
        if self.nb_test_epochs < self.pre_test_epoch_idx:
            ...
            if (self.episode_nb < 250) or (np.random.random() < self.random_task_ratio):
                ...
                new_task = np.random.multivariate_normal(self.alpgmm.gmm.means_[idx],
    ...
    def update_teacher_policy(self):
        ...
        if self.nb_test_epochs < self.pre_test_epoch_idx:  # pre-test phase, only use alp-gmm
            self.alpgmm.history.history.append((task, reward))
            self.alpgmm.update_teacher_policy()
            self.is_new_alpgmm = ((len(self.alpgmm.tasks) % self.alpgmm.fit_rate) == 0)
        ...
        if self.use_alpgmm and (
                self.expert_type == "R" or self.expert_type == 'T') and self.stop_R and self.expert_idx == (
                len(self.expert_means) - 1) and self.nb_alpgmm_gaussians is not None:
        ...
        if self.use_alpgmm and (
                self.expert_type == "stoppedR" or self.expert_type == 'stoppedT') and self.stop_R and self.expert_idx == (
                len(self.expert_means) - 1) and self.nb_alpgmm_gaussians is not None:
            if self.nb_eps_after_R == 0:  # first time, init last IN GMM gaussian tracking to update ALP periodically
                self.last_IN_gaussians_alps = [deque(maxlen=100) for _ in
                                               range(len(self.current_means) - self.nb_alpgmm_gaussians)]
                self.added_since_fit = 0
                assert ((len(self.current_means) - self.nb_alpgmm_gaussians) == len(self.expert_means[-1]))
                print('TIME TO START POST IN, last expert has len {} -->  {}'.format(len(self.expert_means[-1]),
                                                                                     self.expert_means[-1]))
        ...
        if (self.expert_type == 'T' and (self.episode_nb % 250) == 0) \
                or (self.expert_type == 'R' and len(self.reward_list) == self.r_list_len and np.mean(
            self.reward_list) >= self.current_mean_r):
    ...
    def send_test_info(self, test_vec, epoch_0=False):
        ...
        if self.nb_test_epochs == self.pre_test_epoch_idx:  # time to find an expert from classroom
            ...
            if self.random_expert:
                ...
                expert_id = get_k_experts(self.current_student_params, test_vec, initial_test_vectors_list,
                                          last_test_vector,
            ...
            print(folder_path)
            self.expert_means, self.expert_covs, self.expert_mean_rewards = load_expert_trajectory(folder_path,
