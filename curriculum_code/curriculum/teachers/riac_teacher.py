...
class RiacTeacher(Teacher):
    def __init__(self, teacher_parameters, environment_parameters):
        ...
        self.maxlen = 200 if "max_region_size" not in teacher_parameters else teacher_parameters["max_region_size"]
        self.alp_window = self.maxlen if "alp_window_size" not in teacher_parameters else teacher_parameters[
            "alp_window_size"]
        ...
        self.nb_split_attempts = 50 if "nb_split_attempts" not in teacher_parameters else teacher_parameters[
            "nb_split_attempts"]

        # Whether task sampling uses parent and child regions (False) or only child regions (True)
        self.sampling_in_leaves_only = False if "sampling_in_leaves_only" not in teacher_parameters else \
            teacher_parameters[
                "sampling_in_leaves_only"]
        ...
        self.minlen = self.maxlen / 20 if "min_reg_size" not in teacher_parameters else teacher_parameters[
            "min_reg_size"]
        ...
        self.min_dims_range_ratio = 1 / 15 if "min_dims_range_ratio" not in teacher_parameters else teacher_parameters[
            "min_dims_range_ratio"]
        ...
        self.discard_ratio = 1 / 4 if "discard_ratio" not in teacher_parameters else teacher_parameters["discard_ratio"]
    ...
    def update_teacher_policy(self):
        ...
        self.add_task_reward(root, task_array, reward)  # Will update self.nodes_to_split if needed
