...
class ConstMixtureTeacher(Teacher):
    ...
    def update_teacher_policy(self):
        ...
        if len(teacher_history) == 1:
            return
        else:
            last_reward = teacher_history[-2][1]
            new_reward = teacher_history[-1][1]
            self.teacher_weights = self.teacher_weights - (self.step_size/len(self.teachers))
            self.teacher_weights[self.chosen_teacher] += self.step_size * np.sign(new_reward - last_reward)
